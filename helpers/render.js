...
function sceneRender(isFrameRender){   
  ...
  var checkinPath = scenePath + "/checkin"
  var remappedCheckinPath = fileMapper.toNativePath(checkinPath)
  var checkinDir = new Dir(remappedCheckinPath)

  var pathForImgRender = checkinPath + "/preview.png"
  var pathForMovRender = checkinPath + "/preview.mov"

  //check export functionality
  if(!canCreateMovie()){
    alert.log("Could not load WebCCExporter, perhaps the OpenH264 library is not installed.")
  }
    ...
    if(!defaultDisplay){
      ...
      render.frameReady.connect(function(frame, frameCel){
        ...
        if (!isFrameRender && canCreateMovie()){
          var filename = checkinPath + "/movie-" + frame + ".png"
      ...
      if(isFrameRender){
        ...
        alert.log( "....done rendering individual frames. Will now assemble movie");
...
function canCreateMovie() {
    return typeof WebCCExporter === "object" && typeof WebCCExporter.exportMovieFromFiles === "function";
}
...
function createMovie(movieFilename, imageFilenames ){
  if (canCreateMovie()){
    ...
    alert.warning("Could not load WebCCExporter, perhaps the OpenH264 library is not installed.");
  }
}
