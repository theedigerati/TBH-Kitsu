...
Previews.prototype.pull = function(){
    ...
    previews = partials.sortObjects(partials.refactorDates(previews))
...
Previews.prototype.load = function() {
    ...
    for(count = 0; count < previews.length; count++){
        ...
        //preview comment
        var previewComment = new QTableWidgetItem(preview.text ? preview.text : "---")
        ...
        //added by
        var date = partials.getFormattedDate(preview.updated_at, true)
        var addedByHtml = "<h4 style='margin: 0;'>"+ preview.person.first_name +"</h4> <p style='margin: 0; margin-top: 3px; font-size: 10px;'>"+ date +"</p>"
    ...
    this.table.setStyleSheet(styles.taskTypesStyles(this.tasksTypes, true) + styles.taskStatusStyles(this.tasksStatus, true) + styles.editedLabel())
}
